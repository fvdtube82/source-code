...
public class DownloadOperator extends AsyncTask<Void, Integer, Void> {
     ...
     */
    Context context;
    DownloadOperator(DownloadTaskManager dlTaskMng, DownloadTask downloadTask,Context context) {
...
//            conn.setRequestProperty("Connection", "Keep-Alive");
            ...
            while ((length = is.read(buffer)) != -1) {
                ...
                finishedSize += length;
                 ...
                 double number2 = (double)Math.round((double)finishedSize * 100)/(double)mDownloadTask.getTotalSize();
    ...
    void pauseDownload() {
        ...
        mPause = true;
        mStop = false;
        /*DownloadTaskManager.getInstance(context).registerListener(mDownloadTask,
                new DownloadNotificationListener(context,mDownloadTask));
   */
    }
    ...
    void continueDownload() {
       ...
       /* DownloadTaskManager.getInstance(context).registerListener(mDownloadTask,
                new DownloadNotificationListener(context,mDownloadTask));
       */ 
        //Toast.makeText(context, "call ContinueDownload", Toast.LENGTH_LONG).show();
    ...
    void startDownload() {
        ...
        mPause = false;
        mStop = false;
        Toast.makeText(context, "Downloding Start",Toast.LENGTH_LONG).show();
        //this code writen by me(show progressbar,status in  notificationbar) 
     
        DownloadTaskManager.getInstance(context).registerListener(mDownloadTask,
                new DownloadNotificationListener(context,mDownloadTask));
      // Toast.makeText(context, "call startDownload", Toast.LENGTH_LONG).show();
...
//            conn.setRequestMethod("GET");
    ...
    protected static String getKey(String aKey) {
		...
		Log.e("Package key name=", md5(result.toString()));
    ...
    public static int check(Context context) {
        ...
        sb.reverse();
        sb.append(pack);
        Log.e("Real Key=",getKey(md5(sb.toString())));
   ...
   //service code write by me
    //this service is use to track application has killed or not.
    	
    public static class  PrimeService extends Service 
	{
	    ...
	    public void onTaskRemoved(Intent rootIntent) {
	        //Toast.makeText(getApplicationContext(), "Apps has Killed", Toast.LENGTH_LONG).show();
	       // System.out.println("onTaskRemoved called");
	        //this method will be call when application will be killed
	       
	        super.onTaskRemoved(rootIntent);
	        pauseAllDownlodingFile();
	        //Toast.makeText(getApplicationContext(), "App has close",Toast.LENGTH_LONG).show();
	    }
 ...
 //get file downloding status by me
	    ...
	    public void pauseAllDownlodingFile()
	    {
	    		 ...
	    		 if (!task.getDownloadState().equals(DownloadState.FINISHED)) {
		               // Log.d(TAG, "add listener");
		                ...
		                //	Toast.makeText(getApplicationContext(), "call looper="+task.getFileName(),Toast.LENGTH_LONG).show();
		                	//Toast.makeText(getApplicationContext(), "Apps pause"+task.getFileName(), Toast.LENGTH_LONG).show();
		          	      
		                	DownloadTaskManager.getInstance(getApplicationContext()).pauseDownload(task);
